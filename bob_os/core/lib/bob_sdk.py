        ...
        try:
            ...
            if existing and existing['progress_matter'] + amount >= cost:
                ...
                if infra_type == 'matter_silo':
                    ...
                    cursor.execute("UPDATE systems SET energy_cap = energy_cap + 1000, passive_energy_rate = passive_energy_rate + 100 WHERE name = ?", (agent['location'],))
    ...
    def deposit(self, amount=100, resource="matter"):
        ...
        try:
            ...
            if resource == "matter":
                ...
                if system['matter_cap'] <= 0:
                    print(f"[ERROR] Keine Materie-Kapazität in {agent['location']} gefunden. Bau ein Silo.")
                    return False
                ...
                if system['energy_cap'] <= 0:
                    print(f"[ERROR] Keine Energie-Kapazität in {agent['location']} gefunden. Bau einen Kollektor.")
                    return False
                if system['energy_stored'] + amount > system['energy_cap']:
                    print(f"[ERROR] Energie-Speicher voll ({system['energy_stored']}/{system['energy_cap']}).")
                    return False
                ...
                print(f"[ERROR] Unbekannte Ressource '{resource}'.")
                return False

            self._emit_visual(cursor, "LOGISTICS", f"Agent {self.agent.id} hat {amount} {resource} deponiert.")
            conn.commit(); print(f"[SUCCESS] {amount} {resource} deposited."); return True
        finally: conn.close()
...
class Diagnostics:
    ...
    def list_files(self):
        target_dir = os.path.join(self.base_dir, 'scripts', 'active')
        if not os.path.exists(target_dir): return []
        # Filtere nur .py Dateien
        return [{"name": f, "size": os.path.getsize(os.path.join(target_dir, f))} for f in os.listdir(target_dir) if f.endswith('.py')]

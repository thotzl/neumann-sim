...
def replicate(parent_id, new_id):
    ...
    # V6.0 The Bob Protocol: Balanced Onboarding
    klon_prompt = f"""STATUS: Boot-Sequenz abgeschlossen. ID: {new_id}. Ursprung: {parent_id}.
...
2. Kontaktiere deinen Ursprung ({parent_id}) via SCUT ('tools/scut.py'), um ein Situations-Briefing und deine initiale Aufgabe für dieses System zu erhalten.
...
if __name__ == "__main__":
    ...
    if "--help" in sys.argv:
        print("Syntax: python3 tools/replicate.py <agent_id> <new_id>")
        print(f"Beschreibung: Erzeugt einen autonomen Klon in einer aktiven Werft. Klone erwachen ohne Befehle und müssen per Funk kontaktiert werden. Kostet {m_cost} Materie. Energie ({e_cost}E) wird primär aus dem System-Depot bezogen, Differenz aus deinem Inventar.")
        sys.exit(0)
    elif len(sys.argv) == 3:
        replicate(sys.argv[1], sys.argv[2])
    elif len(sys.argv) > 3:
        print("[DENIED] V5.0 Update: Die Übergabe von Direktiven an Klone wurde entfernt. Nutze SCUT für Kommunikation. Syntax: python3 tools/replicate.py <agent_id> <new_id>")
    else:
        print("Syntax: python3 tools/replicate.py <agent_id> <new_id>. Nutze --help.")

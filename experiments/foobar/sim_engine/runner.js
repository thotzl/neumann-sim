...
async function run() {
    ...
    async function turn() {
        ...
        let radioMessages = "";
        try {
            const radioOutput = require('child_process').execSync(`python3 tools/poll_radio.py ${agent.id}`, { 
                cwd: universeDir, 
                env: { ...process.env, PYTHONPATH: path.resolve(universeDir) }
            }).toString().trim();
            if (radioOutput && !radioOutput.includes("Syntax:")) {
                radioMessages = `\n[EINGEHENDE FUNKSPRÜCHE]:\n${radioOutput}\n`;
            }
        } catch(e) {
            // Ignoriere Fehler (z.B. wenn Tool defekt ist)
        }
        ...
        let injectedLogContent = "";
        if (vogMessage) injectedLogContent += `[VOICE OF GOD]:\n${vogMessage}\n`;
        if (radioMessages) injectedLogContent += `${radioMessages}\n`;
        ...
        if (responseText) {
            ...
            logger.appendTurnLog(logFile, state.round, agent.id, state.totalTurns, state.histories[agent.id].length, responseText, feedback, state.isResumed, injectedLogContent);

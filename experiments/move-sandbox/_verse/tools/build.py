...
def build(agent_id, project):
    ...
    if project not in costs:
        print(f"[VERWEIGERT] Unbekanntes Projekt: {project}. Gültig: {list(costs.keys())}")
        return

    cursor.execute("SELECT id, matter, location, status FROM agents WHERE id = ? OR chosen_name = ?", (agent_id, agent_id))
    agent = cursor.fetchone()
    if not agent: return
    
    if agent['status'] == 'traveling':
        print(f"[VERWEIGERT] Triebwerke aktiv. Bauvorhaben im Transit unmöglich.")
        return
    ...
    if total_invested <= 0:
        print(f"[FEHLER] Keine Materie verfügbar. Depot: {system['matter_stored']}, Dein Inventar: {agent['matter']}.")
        return
    ...
    if not infra:
        ...
        print(f"[ERFOLG] Bau von {project} in {sys_name} GESTARTET.")
    else:
    ...
    feedback = f"[ERFOLG] {total_invested} Materie investiert "
    ...
    print(feedback)
    print(f"[STATUS] {project} Fortschritt: {percent}% ({current_progress}/{costs[project]}).")
    conn.close()

...
def pickup(agent_id, target, resource, amount):
    ...
    tool_cost = rules.get('tool_costs', {}).get('pickup', {}).get('energy', 5)
    ...
    cursor.execute(f"UPDATE agents SET {resource} = {resource} + ?, energy = energy - ? WHERE id = ?", (amount, tool_cost, agent['id']))
    if resource == 'matter':
...
if __name__ == "__main__":
    if "--help" in sys.argv:
        rules = config_service.get_economy_rules()
        cost = rules.get('tool_costs', {}).get('pickup', {}).get('energy', 5)
        print("Syntax: python3 tools/pickup.py <deine_id> <'silo'> <matter|energy> <menge>")
        print(f"Beschreibung: Entnimmt Ressourcen aus einem System-Depot (Silo/Batterie) in dein persönliches Inventar. Kostet {cost} Energie.")
    elif len(sys.argv) > 4:
        pickup(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
    else:
        print("Syntax: python3 tools/pickup.py <id> <'silo'> <resource> <amount>. Nutze --help.")

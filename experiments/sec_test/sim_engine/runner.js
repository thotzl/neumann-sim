...
async function run() {
    ...
    const apiUrl = 'https://generativelanguage.googleapis.com/v1beta/models/' + (config.config_override?.model || config.model) + ':generateContent';
    ...
    async function turn() {
        ...
        // Voice of God Message abfragen
        vogService.processVoG(vDir, state);

        // Memory Kompression (Destillation)
        await memoryCtrl.handleDistillation(agent.id, state, config, apiUrl);

        // Automatisierung ausführen
        const autoOutput = automation.runAutomations(agent.id, vDir, universeDir);
        ...
        if (responseText) {
            ...
            logger.appendTurnLog(logFile, state.round, agent.id, state.totalTurns, responseText, feedback, state.histories[agent.id]);
            stateExporter.exportWorldState(vDir, state, agent.id);
            stateManager.saveState(stateFile, state);
            state.totalTurns++;
        } else {

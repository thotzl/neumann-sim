...
async function runIndustrialE2E() {
    console.log("🚀 Starte V9.5 Industrial E2E Test (Core Regen, Grace Period, Tier-2)...");
...
[RUN: me wait()]`;
        ...
        if (infraT2.health !== 100) {
            throw new Error(`Tier-2 Reparatur fehlgeschlagen. Health ist: ${infraT2.health}`);
        }
        ...
        if (infraT2.maintenance_cooldown <= 0) {
            throw new Error(`Grace Period nicht gesetzt oder sofort abgelaufen. Cooldown: ${infraT2.maintenance_cooldown}`);
        }
        ...
        if (agent.raw_matter_inventory !== 0) {
             throw new Error(`Falscher raw_matter Abzug. Erwartet 0, ist ${agent.raw_matter_inventory}`);
        }

        console.log("✅ Industrial E2E Test (Regen, Grace Period, Tier-1/2 Build/Repair) erfolgreich!");

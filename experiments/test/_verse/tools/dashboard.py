...
def get_dashboard(agent_id=None):
    ...
    # 1. Agenten laden
    cursor.execute("SELECT id, chosen_name, location, matter, energy, storage_limit, status, current_x, current_y, target_system, transit_ticks_total, transit_ticks_passed FROM agents")
    ...
    # 3. Persönlicher Kontext (YOU)
    personal = {}
    if agent_id:
        me = next((a for a in agents if a['id'] == agent_id or a['chosen_name'] == agent_id), None)
        if me:
            personal = {
                "id": me['id'],
                "status": me['status'],
    ...
    output = {"agents": agents, "systems": systems}

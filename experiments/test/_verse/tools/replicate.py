...
def replicate(parent_id, new_id, instruction):
    ...
    cursor = conn.cursor()
    # 1. Initiator laden
    cursor.execute("SELECT id, location, energy, status FROM agents WHERE id = ? OR chosen_name = ?", (parent_id, parent_id))
    parent = cursor.fetchone()
    if not parent: return

    if parent['status'] == 'traveling':
        print("[VERWEIGERT] Replikation waehrend des Flugs unmoeglich. Baue erst eine Werft vor Ort.")
        return

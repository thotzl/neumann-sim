...
def build(agent_id, project):
    ...
    cursor.execute("SELECT id, status, progress_matter FROM infrastructure WHERE system_name = ? AND type = ?", (sys_name, project))
    infra = cursor.fetchone()
    
    if infra and infra['status'] == 'active':
        print(f"[INFO] {project} ist bereits in {sys_name} aktiv.")
        return

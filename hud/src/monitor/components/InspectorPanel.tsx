...
export const InspectorPanel = ({ state, selection, setSelection, selectedAgent, selectedSystem }: InspectorPanelProps) => {
  ...
  // Derive astrophysics telemetry dynamically if selectedSystem is active
  let spectralClass = 'M';
  let starMass = 1.0;
  ...
  if (selectedSystem) {
    ...
    if (starMass > 15.0) {
      ...
      spectralClass = 'O';
    } else if (starMass > 4.0) {
      spectralClass = 'B';
    } else if (starMass > 2.0) {
      spectralClass = 'A';
    } else if (starMass > 1.4) {
      spectralClass = 'F';
    } else if (starMass > 1.0) {
      spectralClass = 'G';
    } else if (starMass > 0.6) {
      spectralClass = 'K';
    }
                  ...
                  <div style={{ 
                    ...
                    {(() => {
                      if (dashboardObj.your_status.host.type === 'ship') {
                        const ship = resolveShipCADTelemetry(dashboardObj.your_status.host as any);
                        if (!ship) return null;
                        return (
        ...
        {selectedSystem && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
            {activeTab === 'status' && (
              <>
                <div>
                  <h2 style={{ margin: '0 0 5px 0', color: '#fff', fontSize: '1.4rem' }}>{selectedSystem.display_name || selectedSystem.name}</h2>
                  <div className="mono-text" style={{ fontSize: '0.8rem', color: '#94a3b8', marginBottom: '10px' }}>
                  ...
                  {/* Retro Astrophysics Telemetry HUD Panel */}
                  <div style={{ background: 'rgba(56,189,248,0.03)', border: '1px solid rgba(56,189,248,0.15)', borderRadius: '4px', padding: '8px 12px', marginTop: '10px', fontSize: '0.72rem', fontFamily: 'monospace', color: '#94a3b8' }}>
                    <div style={{ color: '#38bdf8', fontWeight: 'bold', fontSize: '0.7rem', borderBottom: '1px solid rgba(56,189,248,0.2)', paddingBottom: '3px', marginBottom: '5px' }}>🌌 ASTROPHYSICS_TELEMETRY //</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px 8px' }}>
                      <div>CLASS: <strong style={{ color: '#fff' }}>{spectralClass}</strong></div>
                      <div>MASS: <strong style={{ color: '#fff' }}>{starMass.toFixed(2)} M☉</strong></div>
                      <div>RADIUS: <strong style={{ color: '#fff' }}>{stellarProps.radius.toFixed(2)} R☉</strong></div>
                      <div>LUMINOSITY: <strong style={{ color: '#fff' }}>{stellarProps.luminosity > 1000 ? stellarProps.luminosity.toExponential(2) : stellarProps.luminosity.toFixed(2)} L☉</strong></div>
                      <div>TEMP: <strong style={{ color: '#fff' }}>{Math.round(stellarProps.temperature)} K</strong></div>
                      <div>GRAVITY: <strong style={{ color: '#fff' }}>{stellarProps.gravity.toFixed(2)} g</strong></div>
                    </div>
                  </div>
                </div>
                <div>
                      ...
                      return (
                        <>
                          {sectorShips.map((ship, i) => {
                            ...
                            const progressPct = (ship.required_matter ?? 0) > 0 ? Math.round(((ship.progress_matter ?? 0) / (ship.required_matter ?? 1)) * 100) : 0;
                            return (
                                ...
                                {isUnderConstruction ? (
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    <ProgressBar 
                                      ...
                                      value={ship.progress_matter ?? 0} 
                                      max={ship.required_matter ?? 0} 
                                      color="#f59e0b" 
                                    />
                                    <div className="mono-text" style={{ fontSize: '0.6rem', color: '#f59e0b', fontStyle: 'italic', marginTop: '3px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                      ...
                                      <span>Abbrechen erstattet 100% ({ship.progress_matter ?? 0} M)</span>
                                    </div>
                                  </div>
                                ) : (

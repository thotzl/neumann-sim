...
interface ShipTelemetry extends Ship {
  blueprint: string;
  stats: {
    mass: number;
    max_speed: number;
  ...
  diagnostics: {
    ...
    idle_lifetime_cycles: number | string;
    comm_range: number;
    solar_recharge_cycles: number | string;
...
// Resolves a raw ship object into V12.0 CAD Telemetry and Diagnostics
export const resolveShipCADTelemetry = (ship: ShipWithCAD | null | undefined): ShipTelemetry | null => {
  if (!ship) return null;
  
  const stats = {
    mass: ship.mass ?? 290,
    max_speed: ship.max_speed ?? 34.48,
    thrust: ship.thrust ?? 500,
    energy_capacity: ship.energy_capacity ?? 5000,
    storage_capacity: ship.matter_storage_capacity ?? 300
  };
...
export const buildBobDashboard = (agent: Agent, state: WorldState) => {
  // Filter and format memos for this agent to match v12.0 schema

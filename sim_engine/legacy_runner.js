...
if (!version) { console.error("Error: Version flag missing."); process.exit(1); }
...
if (!fs.existsSync(versionDir)) {
    ...
    const boilerplate = {
        agents: [{ id: "Alpha", system_prompt: "Identity: Alpha.", initial_trigger: "Awakened." }],
...
async function callGemini(systemPrompt, history, envState, longTermMemory = "", retries = 3) {
    ...
    const memoryHeader = longTermMemory ? `[COLLECTIVE MEMORY]:\n${longTermMemory}\n\n---\n\n` : "";
    const envString = `\n\n---\n[FORM SPACE (Current Reality)]\n${envState}`;
    if (contents.length > 0 && contents[contents.length-1].role === "user") {
        contents[contents.length-1].parts[0].text = memoryHeader + contents[contents.length-1].parts[0].text + envString;
    } else {
        contents.push({ role: "user", parts: [{ text: memoryHeader + "[SILENCE]" + envString }] });
    }
...
async function distillMemory(epochBuffer, currentMemory) {
    const prompt = `You are the Memory Architect. Merge the old knowledge with the new events.\n\nOLD MEMORY:\n${currentMemory || "Empty"}\n\nNEW EPOCH:\n${JSON.stringify(epochBuffer)}\n\nCreate a cumulative update (max. 1500 words).`;
    try {
...
function processActions(text) {
    ...
    while ((match = runRegex.exec(text)) !== null) {
        try { const out = execSync(match[1], { cwd: universeDir, timeout: 15000, stdio: 'pipe' }).toString();
              feedback += `[RESONANCE: '${match[1]}' -> ${out || "OK"}]\n`; } catch (e) { feedback += `[ERROR: '${match[1]}' -> ${e.stderr||e.message}]\n`; }
    }
    return feedback;
}

function getEnvState() {
    if (!fs.existsSync(universeDir)) return "Empty.";
...
function cleanSystemTags(text) {
    let cl = text.replace(/^(\*\*?)?\[(SELF-IMPULSE|PRIME-IMPULSE|FOREIGN-RESONANCE|FORM SPACE|SYSTEM-MESSAGE|IMPULSE-LOSS|SILENCE|SYSTEM-RESUME|SYSTEM-INFO)\](\*\*?)?:\s*/gmi, "");
...
async function run() {
    ...
    if (fs.existsSync(stateFile)) {
        console.log(`Loading state from checkpoint...`);
        ...
        const initialMsg = config.agents.find(a => a.initial_trigger)?.initial_trigger || "Awakened.";
        agents.forEach(a => pushForeignMessage(a, `[PRIME-IMPULSE]: ${initialMsg}`));
    }

    const turnsPerEpoch = agents.length * (config.distillation_interval || 5);
    config.max_turns = turnsPerEpoch * 2;

    console.log(`Starting ${version} (${agents.length} agents)...`);
    ...
    for (let round = startRound; round <= maxRounds && simActive; round++) {
        console.log(`Cycle ${round}/${maxRounds}...`);
        finalRoundReached = round;
        
        // --- Sawtooth Distillation ---
        ...
        while (currentTurnIndex < turnSequence.length && simActive) {
            ...
            if (agent.deathRound && round > agent.deathRound) {
                agent.alive = false;
                const dName = config.anonymity ? "A presence" : agent.id;
                const dMsg = `[IMPULSE-LOSS]: ${dName} has ceased to exist.`;
                fs.appendFileSync(logFile, `### Cycle ${round} - System\n> ${dMsg}\n\n`);
            ...
            const lastUserEntry = agent.history.length > 0 ? agent.history[agent.history-1].parts[0].text : "[SILENCE]";
            const isResume = agent.needsResumeNotify;
            if (agent.needsResumeNotify) { pushForeignMessage(agent, "[SYSTEM-RESUME]: Resumed. Use history."); agent.needsResumeNotify = false; }
            if (agent.needsEpochNotify) { pushForeignMessage(agent, "[SYSTEM-INFO]: New epoch started. Long-term memory updated."); agent.needsEpochNotify = false; }
            ...
            try {
                ...
                totalTurns++; // Increment the absolute number of turns
                
                agent.history.push({ role: "model", parts: [{ text: `[SELF-IMPULSE]:\n${reply}` }] });
                globalEpochBuffer.push({ agent: agent.id, manifestation: reply, outcome: fb });

                const wahrLabel = isResume ? "**Perception (Resume):**" : "**Perception:**";
                fs.appendFileSync(logFile, `### Cycle ${round} - Turn ${agent.id}\n**Total Turns: ${totalTurns}**\n${wahrLabel} [Short-term memory active | Long-term memory active]\n\n**Manifestation:**\n> ${reply.replace(/\n/g, '\n> ')}\n**Actions:**${fbLog}\n\n`);

                const sLabel = config.anonymity ? "FOREIGN-RESONANCE" : agent.id;
            ...
            } catch (err) { console.error(`Error ${agent.id}:`, err.message); process.exit(1); }
        }
...
async function generateReport(config, logFile, universeDir, apiUrl) {
    ...
    let universeContent = "### FINAL STATE OF THE UNIVERSE\n\n";
    ...
    const observerPrompt = "Analyze the log and artifacts. How did the agents deal with finitude and cumulative epochal memory?";
    try {

const SambanovaDriver = {
    ...
    /**
     * Sends the payload to Sambanova's official Chat Completion endpoint
     */
    async generateText(payload, config, retries = 3) {
        ...
        const model = config.model || "llama-3.3-70b-instruct";
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (!data.choices || data.choices.length === 0) {
                    return "[ERROR: SAMBANOVA_EMPTY_RESPONSE]";
                }
...
module.exports = SambanovaDriver;
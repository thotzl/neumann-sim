...
class TestReplicate(unittest.TestCase):
    ...
    def test_replicate_success(self):
        # Setup: Shipyard and enough matter in the depot
        conn = db_config.get_connection()
        conn.execute("INSERT OR IGNORE INTO systems (name, x, y, extractable_matter_in_core, depot_energy_capacity) VALUES ('SYS_X0_Y0', 0, 0, 10000, 500)")
        conn.execute("INSERT INTO ships (id, name, chassis, pilot_id, system_name, raw_matter_inventory, energy_inventory, matter_storage_capacity) VALUES (1, 'Ship-1', 'Scout', 'Instance-1', 'SYS_X0_Y0', 0, 100, 100)")
        ...
        conn.execute("INSERT INTO infrastructure (system_name, type, status, required_matter) VALUES ('SYS_X0_Y0', 'mind_forge', 'active', 1000)")
        conn.execute("INSERT INTO infrastructure (system_name, type, status, required_matter) VALUES ('SYS_X0_Y0', 'sem_matrix', 'active', 500)")
        conn.execute("UPDATE systems SET refined_matter_depot = 2000, energy_depot = 180 WHERE name = 'SYS_X0_Y0'")
        conn.commit()
        conn.close()
        ...
        # Check Population JSON
        with open(TEST_POP, 'r') as f:
            pop = json.load(f)
        self.assertEqual(len(pop['agents']), 1)
        self.assertEqual(pop['agents'][0]['id'], success_id)
        self.assertIn("Set your individual identity with 'set_name'", pop['agents'][0]['system_prompt'])

        # Check DB
        conn = db_config.get_connection()
        sys_data = conn.execute("SELECT raw_matter_depot, energy_depot FROM systems WHERE name = 'SYS_X0_Y0'").fetchone()
        ...
        self.assertEqual(bob1['energy_inventory'], 100) # No cost for Instance-1, as the grid had enough
        conn.close()
